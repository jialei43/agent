...
class Add(BaseModel):
    ...
    def invoke(self, args):
        # 验证参数
        # self.__class__ ： 当前类的类名 ，就是 Add
        # Add(**args) -> Add(a=1,b=2)  -> tool_instance
        # tool_instance: 参数a=1，b=2构造的Add对象
...
try:
    ...
    if hasattr(ai_msg, 'tool_calls') and ai_msg.tool_calls:
        for tool_call in ai_msg.tool_calls:
            ...
            # 实例化工具类并调用 invoke
            tool_class = selected_tool(**tool_call["args"])
            # 实现思路：提前在invoke方法里面构造对应的工具类(比如Add/Multiply)的对象
            # 然后调用对应的计算方法，返回结果
            # TODO 难点，了解即可
            # tool_instance = tool_class()
            # tool_output = tool_instance.tool_output(tool_call["args"])

            tool_output = tool_class.invoke(tool_call["args"])
