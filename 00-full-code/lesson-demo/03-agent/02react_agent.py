"""
需求：实现ReAct（Reasoning and Acting）模式，让Agent能够边推理边行动
思路步骤：
1. 创建大语言模型实例，配置API参数
2. 定义可调用的工具函数（如乘法运算、天气查询等）
3. 构建ReAct风格的提示模板，引导Agent进行推理和行动
4. 创建ReAct Agent，使其具备推理和行动交替进行的能力
5. 创建Agent执行器，负责执行推理和工具调用过程
6. 测试Agent对不同类型问题的处理能力
"""
