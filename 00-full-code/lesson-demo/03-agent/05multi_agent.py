"""
需求：实现多智能体协作模式，不同专业Agent协同完成复杂任务
思路步骤：
1. 创建大语言模型实例，配置API参数
2. 定义不同功能的工具函数（计算类、信息查询类等）
3. 创建专业Agent（如计算专家、信息专家），各自拥有特定工具集
4. 构建多Agent协作工作流，分配任务给对应专业Agent
5. 整合各Agent结果，生成最终统一回答
6. 测试多Agent协作对复杂任务的处理能力
"""
