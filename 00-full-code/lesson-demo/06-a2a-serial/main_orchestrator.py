"""
需求：实现A2A串行任务编排器，协调多个专业代理完成复合任务
思路步骤：
1. 初始化AgentNetwork并注册多个专业代理（如天气代理、票务代理）
2. 实现串行任务流，按顺序执行多个任务
3. 将前一个任务的输出作为后一个任务的输入
4. 处理任务结果并解析artifacts中的信息
5. 使用异步方式运行编排器程序
"""
...
if __name__ == "__main__":
    # 请确保 weather_agent.py 和 ticket_agent_plus.py 正在运行...
    asyncio.run(main())