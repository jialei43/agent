...
class WeatherAgentServer(A2AServer):
    ...
    def handle_task(self, task):
        ...
        # TODO 这里简化了决策，实际生产级别的Agent这里是使用大模型进行判断
        if "天气" in query:
            ...
            try:
                ...
                task.artifacts = create_artifacts(weather_result)
            except Exception as e:
                error_msg = f"调用 工具失败: {e}"
                print(f"[{self.agent_card.name} 日志] {error_msg}")
                task.artifacts = create_artifacts(error_msg)
        else:
            task.artifacts = create_artifacts("无法理解的任务")
...
def create_artifacts(message):
    return  [{"parts": [{"type": "text", "text": message}]}]
