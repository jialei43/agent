...
"""
    ...
    def generate(self, incident_info: str, previous_draft: str = "", revision_guidance: str = "") -> str:
        ...
        if not previous_draft:
            # ── [旧] 规则引导版：固定7节报告结构，无论信息是否充分都要填满所有章节 ──
            # 缺点：输入信息不足时 LLM 倾向编造数据来凑章节，降低报告可信度；
            #       所有事故用同一模板，无法突出本次事故最关键的维度
            # messages = [
            #     SystemMessage(content="""你是一个资深的 SRE 工程师...
            # 报告结构：
            ...
            # ── [新] 动态推理版：基于现有信息的完整程度自主决定报告结构和侧重点 ──
            # 核心改变：去掉强制7节结构，让 LLM 判断哪些维度有充足数据可以深入分析、
            # 哪些需要标注"信息待补充"，从而避免因信息不足而虚构内容
            messages = [
                SystemMessage(content="""你是一个资深的 SRE 工程师，负责撰写专业的事故复盘报告（Post-Mortem）。

根据提供的事故信息，自主判断哪些维度有足够数据可以深入分析，哪些需要标注"信息待补充"。
一份真实可信的报告比章节齐全更重要：有数据的地方给出具体数字，没有数据的地方如实说明。

核心质量标准（Critic 会按此评分）：
- 根因分析：需追溯到系统性原因，而非止于"人为错误"或"操作失误"
- 影响量化：尽量用数字描述（用户数/时长/损失），无数据时明确标注
- 行动项：必须具体可执行，有负责人和完成标准，避免"加强监控"这类模糊表述
- 时间线：如有时间信息则精确到分钟，信息缺失时说明"""),
                HumanMessage(content=f"请根据以下事故信息撰写复盘报告：\n\n{incident_info}"),
            ]
        else:
            # 基于反馈修订
